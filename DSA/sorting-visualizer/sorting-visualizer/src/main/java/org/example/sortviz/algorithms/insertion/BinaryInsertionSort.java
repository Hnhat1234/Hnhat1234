...
public class BinaryInsertionSort<T> implements SortingAlgorithm<T> {
    ...
    public String getName() {
        return "Xen vào nhị phân (Binary Insertion)";
    }

    @Override
    public AlgorithmCategory getCategory() {
        return AlgorithmCategory.INSERTION;
    }
    ...
    public void sort(List<T> a, Comparator<? super T> cmp, StepEmitter<T> e) {
        ...
        for (int i = 1; i < a.size(); i++) {
            ...
            while (lo < hi) {
                ...
                if (cmp.compare(key, a.get(mid)) < 0)
                    hi = mid;
                else
                    lo = mid + 1;
            }
