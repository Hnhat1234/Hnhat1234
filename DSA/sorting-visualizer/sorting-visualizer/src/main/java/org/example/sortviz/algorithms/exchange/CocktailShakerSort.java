...
public class CocktailShakerSort<T> implements SortingAlgorithm<T> {
    ...
    public String getName() {
        return "Đổi chỗ lắc (Cocktail Shaker)";
    }

    @Override
    public AlgorithmCategory getCategory() {
        return AlgorithmCategory.EXCHANGE;
    }
        ...
        while (swapped) {
            ...
            for (int j = start; j < end; j++) {
                ...
                if (cmp.compare(a.get(j), a.get(j + 1)) > 0) {
                    swap(a, j, j + 1, e, "Đổi chỗ");
                    swapped = true;
                }
            }
            markFinal(end, e, "Cố định phía phải");
            end--;
            if (!swapped)
                break;
            swapped = false;
            for (int j = end; j > start; j--) {
                ...
                if (cmp.compare(a.get(j - 1), a.get(j)) > 0) {
                    swap(a, j - 1, j, e, "Đổi chỗ");
                    swapped = true;
                }
            }
            markFinal(start, e, "Cố định phía trái");
            start++;
        }
        for (int k = start; k <= end; k++)
            markFinal(k, e, "Hoàn tất");
    }
}
