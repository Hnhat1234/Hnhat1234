...
public class BarPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (finalMask != null && i < finalMask.length && finalMask[i])
                col = new Color(60, 180, 90);
            if (last != null) {
                if (last.type == StepType.COMPARE && (i == last.i || i == last.j))
                    col = new Color(230, 200, 80);
                if (last.type == StepType.SWAP && (i == last.i || i == last.j))
                    col = new Color(220, 80, 80);
                if (last.type == StepType.SET && i == last.i)
                    col = new Color(90, 140, 220);
            }
