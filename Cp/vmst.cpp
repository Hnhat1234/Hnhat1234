...
int used[1005][1005], par[1005];

int Rank[1005], check[1005][1005];
...
int main(){
    ...
    for (int i = 0;i < m;++i){
        int u = adj[i].first; int v = adj[i].second;
        if (!dsu(u,v)){
            st.push_back({u,v});
            adj1[u].push_back(v);
            adj1[v].push_back(u);
    ...
    for (int i = 0;i <= n && k < 3;++i){
        if (i == 0){
            ...
            if (Rank[a] > Rank[b]){swap(a,b);}
            for (int i = 0;i < st.size();++i){
                if (st[i].first == a && st[i].second == parent[a]){continue;}
                if (st[i].first == parent[a] && st[i].second == a){continue;}
                cout << st[i].first << ' ' << st[i].second << '\n';
            }
            cout << a << ' ' << b << '\n';
            if (b == par[1]){
                for (int i = 0;i < st.size();++i){
                    if (st[i].first == par[1] || st[i].second == par[1]){continue;}
                    cout << st[i].first << ' ' << st[i].second << '\n';
                }
                cout << a << ' ' << b << '\n';
                break;
               ...
               for (int i = 0;i < st.size();++i){
                    if (st[i].first == b && st[i].second == parent[b]){continue;}
                    if (st[i].first == parent[b] && st[i].second == b){continue;}
                    cout << st[i].first << ' ' << st[i].second;
               }
               cout << a << ' ' << b << '\n';
               break;
