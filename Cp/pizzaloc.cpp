...
pair<int,int> truepos[25];

pair<pair<int,int>,int> customers[25];

int dp[(1 << 20) + 5], d[25];

int sqr(int a){return a * a;}

void cost_prepare(int n){
...
int main(){
    ...
    int m; cin >> m;
    for (int i = 0;i < m;++i){
        cin >> truepos[i].first >> truepos[i].second;
    }
    int n; cin >> n;
    for (int i = 0;i < n;++i){
        cin >> customers[i].first.first >> customers[i].first.second >> customers[i].second;
    }

    for (int i = 0;i < m;++i){
        for (int j = 0;j < n;++j){
            if (sqr(truepos[i].first - customers[i].first.first) + sqr(truepos[i].second - customers[i].first.second) <= r * r){
