...
ll up[100005], a[100005], d[100005];

ll st[17][100005][5], lg2[100005];

void BuildLog2Array() {
    lg2[1] = 0;
    for (int i = 2; i <= 100005; ++i){lg2[i] = lg2[i / 2] + 1;}
}
void preprocess(int n){
    for (int i = 1; i <= n; ++i){st[0][i][0] = st[0][i][1] = a[i]; st[0][i][3] = up[i]; st[0][i][2] = d[i];}
    for (int j = 1; j <= 17; ++j){
        for (int i = 1; i + (1 << j) - 1 <= n; ++i){
            st[j][i][0] = max(st[j - 1][i][0], st[j - 1][i + (1 << (j - 1))][0]);
            st[j][i][1] = min(st[j - 1][i][1], st[j - 1][i + (1 << (j - 1))][1]);
            st[j][i][2] = __gcd(st[j - 1][i][2],st[j - 1][i + (1 << (j - 1))][2]);
            st[j][i][3] = max(st[j - 1][i][3],st[j - 1][i + (1 << (j - 1))][3]);
        }
    }
}
...
ll query(int l,int r,int t){
    ll k = lg2[r - l + 1];
    if (t == 0){return max(st[k][l][0], st[k][r - (1 << k) + 1][0]);}
    else if (t == 1){return min(st[k][l][1], st[k][r - (1 << k) + 1][1]);}
    else if (t == 2){return __gcd(st[k][l][2], st[k][r - (1 << k) + 1][2]);}
    else if (t == 3){return max(st[k][l][3], st[k][r - (1 << k) + 1][3]);}
}
...
int main(){
    ...
    while (q--){
        int l,r; cin >> l >> r;
        if (l == r){cout << "NO";}
        else if (query(l,r,3) >= l){cout << "NO";}
        else{
            ll ma = query(l,r,0);
            ll mi = query(l,r,1);
            ll ggcd = query(l,r - 1,2);
            if ((ma - mi) % (r - l) != 0){cout << "NO";}
            else{
                ll d = (ma - mi)/(r - l);
                cout << (ggcd == d ? "YES" : "NO");
            }
