...
int main(){
    ...
    for (int i = 1;i <= n;++i){cin >> b[i]; prefb[i] = preb[i - 1] + a[i];}
        ...
        for (int y = x;y <= n;++y){
            ...
            for (int i = 1;i <= n;++i){
                ...
                if (i >= x){
                    int l = x;
                    int r = i;
                    if (r <= y){val += da[r] - da[l - 1] - (r - l + 1) * prefa[l - 1];}
                    else{
                        val += da[y] - da[l - 1] + prefa[r] - prefa[y] - (y - l + 1) * prefa[l - 1] - (r - y) * prefa[y];
                    }
                }
                if (i <= y){
                    int l = i;
                    int r = y;
                    if (l >= x){val += db[r] - db[l];}
                    else{
                        val += db[r] - db[l - 1] - (r - l + 1) * prefb[l - 1] + prefb[x - 1] - prefb[l - 1];
                    }
                }
                sum += val;
                if (i == 1){ans = max(ans,sum);}
                if (i > 1){ans max(ans,sum - mi);}
                mi = min(mi, val);
            }
        }
    }
