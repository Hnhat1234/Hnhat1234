...
bool cmp(ii a, ii b){
    return ((t[a.first] + t[a.second]) < (t[b.first] + t[b.second]));
}
int get(int x){
...
int main(){
    ...
    int n; cin >> n;
    for (int i = 1;i <= n;++i){cin >> t[i]; par[i] = i; mx[i] = t[i];}
    for (int i = 1;i < n;++i){
        int x , y; cin >> x >> y;
        adj.push_back({x,y});
    }
    ...
    ll ans = 0;
    for (int i = 0;i < n - 1;++i){
        ans += dsu(adj[i].first,adj[i].second);
    }
    cout << ans;
}
