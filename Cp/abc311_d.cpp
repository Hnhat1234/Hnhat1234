...
ii calc(int x,int y,int dirx,int diry,int n,int m){
    ii base = {0,0};
    int cur = 0;
    while (!vis[x][y] && c[x][y] != '#' && x > 0 && y > 0 && x <= n && y <= m){
        if (!vis[x][y] && c[x][y] != '#' && x + dirx > 0 && y + diry > 0 && x + dirx <= n && y + diry <= m){
            if (c[x + dirx][y + diry] == '#' && cur > 0){
...
int main(){
    ...
    while (q.size()){
        ...
        ii dir1 = calc(x,y,1,0,n,m);
        ii dir2 = calc(x,y,0,1,n,m);
        ii dir3 = calc(x,y,-1,0,n,m);
        ii dir4 = calc(x,y,0,-1,n,m);
        if (dir1.first != 0){q.push(dir1);}
        if (dir2.first != 0){q.push(dir2);}
        if (dir3.first != 0){q.push(dir3);}
        if (dir4.first != 0){q.push(dir4);}
    }
    int ans = 0;
    for (int i = 1;i <= n;++i){
        for (int j = 1;j <= m;++j){
            if (c[i][j] == '.' && vis[i][j]){ans++;}
        }
    }
    cout << ans;
}
