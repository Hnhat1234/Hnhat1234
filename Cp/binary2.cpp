...
ll dp[1000005][2], pref[1000005][2];

ll calc(int l,int r,int t,ll mod){
    return (pref[r][t] - pref[l - 1][t] + mod) % mod;
}
int main(){
    ...
    for (int i = 1;i <= n;++i){
        if (i == 1){
            ...
            (dp[i][0] += calc(max(i - k + 1,1),i - 1,1,mod)) %= mod;
            (dp[i][1] += calc(max(i - k + 1,1),i - 1,0,mod)) %= mod;
            if (i - k + 1 < 0){
                (dp[i][0] += 1) %= mod;
                (dp[i][1] += 1) %= mod;
            }
