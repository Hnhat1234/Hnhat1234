...
ll d[2005][2005]; int vis[2005][2005];
int main(){
    ...
    for (int i = 1;i <= h;++i){
        int t = 0;
        for (int j = 1;j <= w;++j){
            if (c[i][j] == '.'){vis[i][j] = max(t,vis[i][j]); continue;}
            if (c[i][j] == '>'){t = 1; continue;}
    ...
    for (int j = 1;j <= w;++j){
        int t = 0;
        for (int z = 1;z <= h;++z){
            if (c[z][j] == '.'){vis[z][j] = max(t,vis[z][j]); continue;}
            if (c[z][j] == 'v'){t = 1; continue;}
            if (c[z][j] != 'v'){t = 0; continue;}
        }
        t = 0;
        for (int z = h;z >= 1;--z){
            if (c[z][j] == '.'){vis[z][j] = max(t,vis[z][j]); continue;}
            if (c[z][j] == '^'){t = 1; continue;}
            if (c[z][j] != '^'){t = 0; continue;}
        }
    }
    ...
    while (q.size()){
        ...
        for (int i = 0;i < 4;++i){
            int dx = x.first + d1[i];
            int dy = x.second + d2[i];
            if (dx <= 0 || dy <= 0 || dx > h || dy > w){continue;}
            if (c[dx][dy] != '.' && c[dx][dy] != 'G'){continue;}
            if (vis[dx][dy]){continue;}
            q.push({dx,dy});
            d[dx][dy] = min(d[dx][dy], d[x.first][x.second] + 1);
        }
    }
    cout << (d[en.first][en.second] == 1e9 ? -1 : d[en.first][en.second]);
}
