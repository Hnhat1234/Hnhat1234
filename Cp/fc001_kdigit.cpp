...
ll dp_a[25][10], dp_b[25][10];

ll f_a[25][10], f_b[25][10];
...
int main(){
    ...
    pw9[0] = 1;
    for (int i = 1;i <= 19;++i){pw9[i] = (pw9[i - 1] * 9LL) % mod;}
    string a, b;
    int k; cin >> k;
    a = ' ' + a; b = ' ' + b;
    for (int i = 1;i <= a.size();++i){
        for (int digit = 0;digit < 10;++digit){
            for (int j = i - 1;j >= 0;--j){
                if (i - j >= k){(dp_a[i][digit] += pw9[j]) % mod; break;}
                else{
                    for (int d = 0;d < 10;++d){
                        if (d == digit){continue;}
                        (dp_a[i][digit] += dp_a[j][d]) %= mod;
                    }
                }
            }
        }
    ...
    for (int i = 1;i <= b.size();++i){
        for (int digit = 0;digit < 10;++digit){
            for (int j = i - 1;j >= 0;--j){
                if (i - j >= k){(dp_b[i][digit] += pw9[j]) % mod; break;}
                else{
                    for (int d = 0;d < 10;++d){
                        if (d == digit){continue;}
                        (dp_b[i][digit] += dp_b[j][d]) %= mod;
                    }
                }
            }
        }
