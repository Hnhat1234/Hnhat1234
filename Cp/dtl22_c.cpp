...
void mem_set(int t,int s,int c){
    for (int i = 0;i <= t + 1;++i){
        for (int j = 0;j <= c;++j){
            for (int z = 0;z <= s;++z){
                dp[i][j][z] = -1;
...
int main(){
    ...
    int t, s; cin >> t >> s;
    int c, k ,d ,l; cin >> c >> k >> l >> d;
    mem_set(t, s, c);
    for (int i = 0;i <= t;++i){
        if (i == 0){dp[0][c][0] = 0;}
        for (int j = 0;j <= c;++j){
            for (int v = 0;v <= s;++v){
                if (dp[i][j][v] == -1){continue;}
                dp[i + 1][min(j + d,c)][0] = max(dp[i + 1][min(j + d,c)][0], dp[i][j][v]);
                if (v > 1){dp[i + 1][min(j + d,c)][v - 1] = max(dp[i + 1][min(j + d,c)][v - 1], dp[i][j][v] + (v - 1));}
                if (j - k >= 0 && v > 0){dp[i + 1][j - k][v] = max(dp[i + 1][j - k][v], dp[i][j][v] + v);}
                if (v < s && j - l >= 0){dp[i + 1][j - l][v + 1] = max(dp[i + 1][j - l][v + 1], dp[i][j][v] + (v + 1));}
            }
        }
    }
    ll res = 0;
