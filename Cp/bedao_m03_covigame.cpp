...
int a[100005], check[100005], pos[100005];

vector<int> p[100005], b;
...
int main(){
    ...
    for (int i = 2;i <= n;++i){
        pos[i] = (abs(a[i] - a[i - 1]) > k);
        total += pos[i];
    }

    int ans = 100001; int x = 100001;

    for (int i = 0;i < b.size();++i){
        int val = total;
        for (int j = 0;j < (int)p[b[i]].size();++j){
            int l = p[b[i]][j] - 1;
            while (j < (int)p[b[i]].size() - 1 && p[b[i]][j] == p[b[i]][j + 1] + 1){j += 1;}
            int r = p[b[i]][j] + 1;
            if (l == 0){
                if (r <= n){val -= pos[r];}
                else{
                ...
                if (pos[l + 1] + pos[r] == 2 && r <= n){
                    if (abs(a[r] - a[l]) > k) val -= 1;
                    if (abs(a[r] - a[l]) <= k) val -= 2;
                }
                if (pos[l + 1] + pos[r] == 0 && r <= n){
                    val += (abs(a[r] - a[l]) > k);
                }
                if (pos[l + 1] + pos[r] == 1 && r <= n){
                    val -= (abs(a[r] - a[l]) <= k);
                }
                if (r > n){
                    val -= pos[l + 1];
