...
ll st_max[300005][25], st_min[300005][25], a[300005];
...
void BuildLog2Array() {
    lg2[1] = 0;
    for (int i = 2; i <= 300005; ++i){lg2[i] = lg2[i / 2] + 1;}
}
...
ll get(ll l,ll r,ll t){
    ll k = lg2[r - l + 1];
    if (t == 0){return max(st_max[l][k],st_max[r - (1 << k) + 1][k]);}
    else if (t == 1){return min(st_min[l][k],st_min[r - (1 << k) + 1][k]);}
}
int main(){
    ...
    for (int i = 1;i <= n;++i){
        ...
        while (l <= r){
            ll mid = (l + r)/2;
            if (get(mid,i,0LL) - get(mid,i,1LL) <= k){
                res = min(res,mid);
                r = mid - 1;
            }else{
                l = mid + 1;
    ...
    for (int i = n;i >= 1;--i){maxsuff[i] = max(maxsuff[i + 1],g[i]);}
    ll ans = 0;
    for (int i = 1;i <= n;++i){
        ans = max(ans,f[i] + maxsuff[i + 1]);
    }
    cout << ans;
}
